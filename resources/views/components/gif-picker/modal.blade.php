<x-modal name="gif-picker" maxWidth="md">
    <div class="flex flex-col max-h-[85vh]" x-data="gifPicker('{{ route('giphy.trending') }}', '{{ route('giphy.search') }}')">
        ...
        <!-- GIF Grid -->
        <div class="flex-1 overflow-y-auto px-4 py-3 min-h-[400px]">
            <div x-show="loading" class="flex items-center justify-center h-[400px]">
                <x-loading-dots size="lg" />
            </div>

            <div x-show="!loading && gifs.length === 0" class="flex flex-col items-center justify-center h-[400px] text-center">
